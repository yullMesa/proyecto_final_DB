-- 1. JOIN: Listado de tickets con nombre de cliente, pel�cula y sala
...
-- 2. AGREGACI�N: Total de boletos vendidos por cada pel�cula (COUNT + GROUP BY)
SELECT 
...
-- 3. AGREGACI�N: Precio promedio, m�nimo y m�ximo de las funciones por sala (AVG, MIN, MAX)
...
-- 2. AGREGACIÓN (Left Join): Películas y ventas (Incluso las que no tienen ventas)
-- Aquí el LEFT JOIN es perfecto para auditoría: ver qué películas no han vendido nada.
...
-- 3. AGREGACIÓN: Resumen de precios por sala (Ya estaba bien, mantenemos Inner)
